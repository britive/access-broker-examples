...
# Aurora MySQL Temporary User Checkout Script
# Creates a temporary MySQL user and grants full access to the target database
...
# Generate random alphanumeric password for temporary user
...
# Grant full privileges on the target database
echo "Granting ALL privileges on systemdb.* to ${MYSQL_USER}@${MYSQL_HOST}" >&2
mysql \
  --defaults-extra-file="$tmp_conf.cnf" \
  -e "GRANT ALL ON systemdb.* TO '${MYSQL_USER}'@'${MYSQL_HOST}';"

if [[ $? -ne 0 ]]; then
    echo "Error: Failed to grant privileges. Cleaning up user..." >&2
    # Attempt to remove the created user before exiting
...
# Output credentials and connection string for the Britive platform
echo "$MYSQL_USER"
echo "$password"
