#!/bin/bash
# =============================================================================
# MongoDB On-Premises JIT Access - Checkin Script (dbAdmin)
# =============================================================================
# Purpose   : Revokes the elevated 'dbAdmin' role and restores the user to
#             the baseline 'read' role on the target database.
...
# ---------------------------------------------------------------------------
# Logging setup — mirrors checkout so both events appear in the same log file.
# ---------------------------------------------------------------------------
LOG_DIR="${LOG_DIR:-/tmp}"
...
# ---------------------------------------------------------------------------
# Normalize username — same logic as checkout to ensure consistent user lookup.
...
log "Starting dbAdmin checkin for '${USERNAME}' on database '${DATABASE}'..."

# ---------------------------------------------------------------------------
# Step 1: Test connectivity before making changes.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 2: Restore the baseline 'read' role, revoking elevated dbAdmin access.
# The Atlas PATCH endpoint replaces all roles, so setting only 'read' here
# effectively removes dbAdmin entirely.
# ---------------------------------------------------------------------------
log "Restoring 'read' role on '${DATABASE}' for '${USERNAME}' (revoking dbAdmin)..."
...
if [ "${PATCH_CODE}" != "200" ]; then
  log_error "Role revocation failed (HTTP ${PATCH_CODE}) for '${USERNAME}'."
  log_error "Response: $(cat "${TMPFILE}")"
  exit 1
fi

log "Role revocation succeeded (HTTP ${PATCH_CODE})."

# ---------------------------------------------------------------------------
# Step 3: Verify the baseline role is now in effect.
# ---------------------------------------------------------------------------
