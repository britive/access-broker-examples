...
# IBM AS400 Group Membership Checkin Script
# This script removes group membership for a user on IBM AS400 system
...
echo "Starting AS400 group membership checkin process"
...
# Destination host and Admin user
HOST="$BRITIVE_AS400_HOST"
ADMN="$BRITIVE_ADMIN_USER"

# Path Admin user SSH Private key identity file for SSH connection
...
echo "Connecting to AS400 system to remove group membership for user: $USER"
...
if ssh -i "$SSH_KEY_PATH" \
       ...
       -o BatchMode=yes \
       $ADMN@$HOST \
       "system 'CALL PGM(FOLD/BRTVRMVGRP) PARM(\"$USER\", \"$GROUP\")'"; then
    echo "SUCCESS: Group membership successfully removed for user $USER"
else
    echo "ERROR: Failed to remove group membership for user $USER" >&2
    exit 1
fi

echo "AS400 group membership checkin completed successfully"