...
remove_user_from_role() {
    ...
    get_user_id
    get_role_id
    # Fetch role assignment ID if available
    ROLE_ASSIGNMENT_ID=$(curl -s \
        -H "Authorization: Bearer $ACCESS_TOKEN" \
        "https://admin.googleapis.com/admin/directory/v1/customer/$CUSTOMER_ID/roleassignments?userKey=$USER_ID" \
        | jq -r ".items[] | select(.roleId == \"$ROLE_ID\") | .roleAssignmentId")
    ...
    if [ -n "$ROLE_ASSIGNMENT_ID" ]; then
        ...
        echo "Removed $IDENTITY from role $ROLE_NAME"
    else
        echo "No role assignment found for $IDENTITY in role $ROLE_NAME"
    fi
}
