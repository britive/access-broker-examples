...
# Remove a user from Google Workspace group
remove_user_from_group() {
    curl -X DELETE \
        -H "Authorization: Bearer $ACCESS_TOKEN" \
        "https://admin.googleapis.com/admin/directory/v1/groups/$GROUP/members/$IDENTITY"
    echo "Removed $IDENTITY from group $GROUP"
}
